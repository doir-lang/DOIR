...
namespace doir {
	struct module : public ecrs::context {
		...
		mutable std::shared_ptr<string_interner> interner = std::make_shared<string_interner>();
		...
		bool flags_set(ecrs::entity_t e, doir::flags::impl check) const {
			return flags_set(e, (decltype(doir::flags{}.as_underlying()))check);
		}
		bool flags_set(ecrs::entity_t e, decltype(doir::flags{}.as_underlying()) check) const;
