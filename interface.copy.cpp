...
namespace doir {
	void copy_components(module& mod, ecrs::entity_t out, ecrs::entity_t subtree, bool copy_block /*= true*/, std::unordered_map<ecrs::entity_t, ecrs::entity_t>* substitutions /*= nullptr*/) {
		...
		if(mod.has_component<doir::type_of>(subtree) || mod.has_component<doir::lookup::type_of>(subtree)) {
			...
			if(mod.has_component<doir::type_of>(subtree)) {
				...
				destination = source;
				if(substitutions) for(auto& e: destination.related)
			...
			if(mod.has_component<doir::lookup::type_of>(subtree)) {
				...
				if(substitutions) destination = lookup.resolved() && substitutions->contains(lookup.entity())
					? doir::lookup::type_of{(*substitutions)[lookup.entity()]} : lookup;
			}
			...
			if(valueless) {
				...
				if(mod.has_component<doir::lookup::call>(subtree)) {
					...
					if(substitutions) destination = lookup.resolved() && substitutions->contains(lookup.entity())
						? doir::lookup::call{(*substitutions)[lookup.entity()]} : lookup;
				}
				...
				if(mod.has_component<doir::lookup::function_inputs>(subtree)) {
					...
					if(substitutions) for(auto& lookup: lookups)
						destination.push_back(lookup.resolved() && substitutions->contains(lookup.entity())
							? doir::lookup::type_of{(*substitutions)[lookup.entity()]} : lookup);
				}
				...
				if(mod.has_component<doir::lookup::function_return_type>(subtree)) {
					...
					if(substitutions) destination = lookup.resolved() && substitutions->contains(lookup.entity())
						? doir::lookup::function_return_type{(*substitutions)[lookup.entity()]} : lookup;
				}
			}
			...
			if(mod.has_component<doir::lookup::alias>(subtree)) {
				...
				destination = lookup.resolved() && substitutions->contains(lookup.entity())
					? doir::lookup::alias{(*substitutions)[lookup.entity()]} : lookup;
			}
		}
	...
	void deep_copy_copy_components(module& mod, ecrs::entity_t out, ecrs::entity_t block, std::unordered_map<ecrs::entity_t, ecrs::entity_t>& substitutions, std::unordered_map<ecrs::entity_t, ecrs::entity_t>& reverse_substitutions, void(*extra_copy_instructions)(ecrs::entity_t dest, ecrs::entity_t src)) {
		auto subtree = reverse_substitutions[out];

		mod.get_or_add_component<doir::parent>(out) = {{block}};

		copy_components(mod, out, subtree, false, &substitutions);

		if(mod.has_component<doir::block>(subtree)) {
			for(auto& e: mod.get_component<doir::block>(out).related)
				deep_copy_copy_components(mod, e, out, substitutions, reverse_substitutions, extra_copy_instructions);
		}
	...
	ecrs::entity_t deep_copy_build_structure(module& mod, ecrs::entity_t subtree, std::unordered_map<ecrs::entity_t, ecrs::entity_t>& substitutions, std::unordered_map<ecrs::entity_t, ecrs::entity_t>& reverse_substitutions) {
		ecrs::entity_t out = mod.add_entity();
		assert(!substitutions.contains(subtree));
		substitutions[subtree] = out;
		reverse_substitutions[out] = subtree;
	...
	ecrs::entity_t deep_copy(module& mod, ecrs::entity_t subtree, void(*extra_copy_instructions)(ecrs::entity_t dest, ecrs::entity_t src) /*= nullptr*/) {
		std::unordered_map<ecrs::entity_t, ecrs::entity_t> substitutions;
		std::unordered_map<ecrs::entity_t, ecrs::entity_t> reverse_substitutions;
		auto block = mod.get_component<doir::parent>(subtree).related[0];
		auto out = deep_copy_build_structure(mod, subtree, substitutions, reverse_substitutions);
		deep_copy_copy_components(mod, out, block, substitutions, reverse_substitutions, extra_copy_instructions);
		return out;
	}



	block_builder& block_builder::move_existing(block_builder& source) {
