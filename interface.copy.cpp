...
namespace doir {
	void deep_copy_second_pass(module& mod, ecrs::entity_t out, std::unordered_map<ecrs::entity_t, ecrs::entity_t>& subtitutions, std::unordered_map<ecrs::entity_t, ecrs::entity_t>& reverse_subtitutions, void(*extra_copy_instructions)(ecrs::entity_t dest, ecrs::entity_t src)) {
		...
		if(mod.has_component<doir::block>(subtree)) {
			for(auto& e: mod.get_component<doir::block>(subtree).related)
				deep_copy_second_pass(mod, e, subtitutions, reverse_subtitutions, extra_copy_instructions);
		}
	...
	ecrs::entity_t deep_copy(module& mod, ecrs::entity_t subtree, void(*extra_copy_instructions)(ecrs::entity_t dest, ecrs::entity_t src) /*= nullptr*/) {
		...
		deep_copy_second_pass(mod, out, subtitutions, reverse_subtitutions, extra_copy_instructions);
		return out;
	}
	...
	block_builder& block_builder::copy_existing(const block_builder& source, bool skip_parameters /*= false*/) {
		...
		for(size_t i = start; i < block.related.size(); ++i)
			deep_copy_second_pass(*mod, block.related[i], subtitutions, reverse_subtitutions, nullptr);
