	...
	struct byte_dumper {
		...
		std::vector<std::byte>& interpret_number_assign(std::vector<std::byte>& out, const doir::module& mod, ecrs::entity_t subtree) {
			if(mod.has_component<doir::number>(subtree)) {
				auto byte = lookup::resolve((doir::module&)mod, *mod.interner->find("compiler.byte"), 1);
			...
			if(mod.has_component<doir::string>(subtree)) {
				auto byte_pointer = lookup::resolve((doir::module&)mod, *mod.interner->find("compiler.byte_pointer"), 1);
		...
		std::vector<std::byte>& interpret_call(std::vector<std::byte>& out, const doir::module& mod, ecrs::entity_t subtree) {
			...
			auto emit = lookup::resolve((doir::module&)mod, mod.interner->intern("compiler.emit"), 1, true);
			auto emit_bytes = lookup::resolve((doir::module&)mod, mod.interner->intern("compiler.emit_bytes"), 1, true);
