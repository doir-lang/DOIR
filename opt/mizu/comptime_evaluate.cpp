...
namespace doir::opt::mizu {
	bool comptime_evaluate(ecrs::context& ctx, ecrs::entity_t subtree, std::function<bool(ecrs::context&)> mizu_schedule) {
		...
		if(comptime_value_available(mod, subtree)) return true;
		...
		static ecrs::entity_t block_ = doir::lookup::resolve(mod, "block", 1, true);
		static ecrs::entity_t void_ = doir::lookup::resolve(mod, "void", 1, true);
		static ecrs::entity_t early_include = doir::lookup::resolve(mod, "early_include", 5, true);
		...
		static ecrs::entity_t mizu_doir_attach_comptime_number_i64 = doir::lookup::resolve(mod, "mizu.doir_attach_comptime_number_i64", 1, true);
		...
		if(mod.has_component<doir::function_inputs>(subtree)) {
			...
			for(i = 0; i < inputs.related.size(); ++i) {
				...
				else if(mod.has_component<doir::type_of>(e) && (mod.get_component<doir::type_of>(e).related[0] == block_ || mod.get_component<doir::type_of>(e).related[0] == type))
					e = comptime_block.push_number(name, mizu_u64, doir::alias::resolve(mod, e));
		...
		auto name = mod.has_component<doir::name>(subtree) ? mod.get_component<doir::name>(subtree) : doir::interned_string::wildcard;
		auto ret = comptime_block.push_call(name, type_of, called_function, arguments);
		r = comptime_block.push_number("_", assembler_register, r_value++);
		{
