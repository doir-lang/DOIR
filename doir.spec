...
assignment <- (<'export'>_)? Identifier _ ':'_ Type wsc (_ '='_ (Constant wsc / Block wsc / function_call / Identifier wsc / Type wsc))? (SourceInfo wsc)? Terminator _

deducible_type <- ('deduced'_)? Type _
identifier_type_pair <- Identifier _ ':'_ deducible_type
FunctionType <- '('_ (identifier_type_pair (','_ identifier_type_pair)*)? ')' (_ '->'_ Type)?
...
Terminator <- ';' | '\n' | '\r\n' | '\r'
Identifier <- !Keywords < ([%]/\p{UnicodeIdentifierStart})([.]/\p{UnicodeIdentifierContinue})* >
...
FloatConstant <- (([0-9]* '.' [0-9]+ / [0-9]+ '.'?) ('e'i[+\-]? [0-9]+)? )
